...
<form class="column is-8 is-offset-2" action="{{ route('books.update',$book->id) }}" method="POST">
    @csrf
    @method('PUT')
    <h3 class="has-text-weight-bold">book名:</h3>
    <input class="input" type="text" name="name" value="{{ $book->name }}" placeholder="book名">
    <h3 class="has-text-weight-bold">詳細:</h3>
    <textarea class="input" style="height:150px" name="detail" placeholder="Body">{{ $book->body }}</textarea>
    <div class="columns">
        <div class="column">
